...
int main()
{
    ...
    grad->width = 6;
    grad->height = 5;
    grad->raster = malloc(sizeof(uint8_t) * 3 * grad->width * grad->height);


    struct rgb_img *im;
    read_in_img(&im, "C:\\Users\\Qiren\\Desktop\\ESC190\\project_two\\6x5.bin");
    calc_energy(im,  &grad);
    print_grad(grad);
    ...
    struct rgb_img *dest;
    remove_seam_test(im, &dest, path);
    write_img(dest, "C:\\Users\\Qiren\\Desktop\\ESC190\\project_two\\6x5_test.bin");
}
